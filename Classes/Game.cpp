...
* set Font
*/
Game::Game() : currentState(MAIN_MENU), player(100.0f, 350.0f)
{
	...
	player.setPositionExternal(sf::Vector2f(1000.0f, 500.0f));
...
void Game::update()
{
	...
	if (trailFiller < 0.3f)
		trailFiller += deltaTime;
	else {
		trailFiller = 0.0f;
		trail.push_back(sf::CircleShape())
	}
...
void Game::renderWinWindow()
{
	sf::Text winText;
	winText.setFont(font);
	winText.setCharacterSize(30);
	winText.setFillColor(sf::Color::Red);
	winText.setString("YOU WON!\nKONGRATULEJSZYNS-you've escaped from the loop - the hackathon");
	winText.setPosition(700.f, 300.f);
